...
<!-- <div class="w3-display-middle"> --><center>
	...
	<form action="<?php echo url('/startmodule/response')?>" method="post">
    <?php echo csrf_field()?>
            <input type="hidden" name="index" value="{{$lessonmodule->index}}">
            <input type="hidden" name="lesson" value="{{$lesson->lesson_name}}">
            <input type="hidden" name="revcenter_ID" value="{{$revcenter}}">
            <h3>{{$lessonmodule->question->question}}</h3>
            <br>
            @foreach ($choices as $choice)
            <h4><input type="radio" name="choice" required value="{{$choice->modulechoice_ID}}"> {{$choice->choice}}</h4><br>
            @endforeach()
    <br><br>   
